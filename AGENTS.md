...
本文适用于整个仓库。详细架构、运行时契约和当前能力以 [`docs/framework-design.md`](docs/framework-design.md) 为准；本文只记录编码 Agent 必须遵守的强约束。不要把规划中或 IDL 中尚未落地的能力描述为已经实现。
...
- `main` 只读：禁止在 `main` 上修改、提交或直接 push，也禁止手工绕过质量门禁。只允许 GitHub 自动创建 `dev -> main` Pull Request，并在检查通过后以 merge commit 合并。
- 禁止 force-push、改写或 rebase 共享的 `dev`、`main` 历史。`dev -> main` 只能由自动 PR 以 merge commit 推进；合并后只允许把 `dev` fast-forward 到 `main`。发生分叉时停止并报告，不得自行丢弃提交。
...
- `internal/client` 封装 Identity RPC，`internal/middleware` 承载跨路由安全策略，`internal/context` 显式装配 Redis、Etcd resolver、Identity client 与 HTTP components。
...
- 长运行服务必须完整实现 `Name`、`Serve`、`Ready(context.Context)`、`Shutdown(context.Context)`。启动 goroutine 不等于 ready；`Ready` 必须证明 listener 和必要的外部注册确实可用。
...
- 契约源是 `idl/http/v1/gateway.thrift` 与 `idl/rpc/v1/*.thrift`；生成输出不是契约源。
- `scripts/generated-files.txt` 是生成器所有权的唯一清单。清单内文件禁止手改，`kitex_gen/` 完全由生成器拥有；Gateway handler 和未列入清单的 route middleware 保持手写/混合所有权。
- 生成只通过仓库脚本或 `make generate` 完成。Hertz 只允许 `hz update`，严禁 `hz new`；使用仓库固定的 Kitex `v0.16.2`、Hertz `v0.9.7`、thriftgo `0.4.5`。
...
- 每次改动完成后运行 `make ci`；它包含格式、vet、lint、无缓存测试、build 和生成漂移检查。
- 修改并发、goroutine、组件编排或资源生命周期时额外运行 `make race`。
- IDL 变更除 `make ci` 外还必须运行上一节的 `compat-git` 检查。
- 修复格式差异应运行仓库格式命令并审查实际语义差异，不得接受整文件无语义的行尾重写。

## 文档与文本

- 公共运行时、配置、契约、依赖边界或生命周期发生变化时，同步更新 [`docs/framework-design.md`](docs/framework-design.md)；面向用户的启动方式或可用 API 变化时同步更新 `README.md`。
