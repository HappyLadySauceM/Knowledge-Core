...
func CORS() app.HandlerFunc {
	return func(ctx context.Context, request *app.RequestContext) {
		...
		if string(request.Method()) == consts.MethodOptions && request.GetHeader("Access-Control-Request-Method") != nil {
			request.Header("Access-Control-Allow-Headers", "Authorization, Content-Type, Idempotency-Key, If-Match, X-Confirm-Permanent-Delete, X-Request-ID, traceparent, tracestate")
