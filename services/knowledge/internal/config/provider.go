...
func (p *Provider) Load(ctx context.Context, command *cobra.Command) (Config, error) {
	if command == nil {
		return Config{}, errors.New("load knowledge configuration: command is required")
	}
	if ctx == nil {
		return Config{}, errors.New("load knowledge configuration: context is required")
	}
	if err := ctx.Err(); err != nil {
		return Config{}, fmt.Errorf("load knowledge configuration: %w", err)
	}
	configFile := strings.TrimSpace(p.configFile)
	if configFile == "" {
		return Config{}, errors.New("load knowledge configuration: --config is required")
	}
	v := viper.New()
	...
	if err := v.UnmarshalExact(&loaded, viper.DecodeHook(configurationDecodeHook())); err != nil {
		return Config{}, fmt.Errorf("decode knowledge configuration: %w", err)
	}
	remoteBootstrap, err := configcenter.BootstrapFromEnvironment(p.defaults.App.Name)
	if err != nil {
		return Config{}, fmt.Errorf("load knowledge dynamic configuration bootstrap: %w", err)
	}
	remote := configcenter.NewManager(remoteBootstrap)
	document, err := remote.Load(ctx)
	if err != nil {
		return Config{}, fmt.Errorf("load knowledge dynamic configuration: %w", err)
	}
	baseline := loaded
	if document != nil {
...
func (p *Provider) BindRuntime(ctx context.Context, runtime *coreapp.Runtime) error {
	if runtime == nil {
		return errors.New("bind knowledge runtime configuration: runtime is required")
	}
	if p.remote == nil || !p.remote.Enabled() {
		return nil
	}
	if p.apply == nil {
		return errors.New("bind knowledge runtime configuration: service applier is required")
	}
...
func applyDocument(current, baseline, startup Config, document configcenter.DynamicDocument) (Config, configcenter.ApplyResult, error) {
	...
	if err := document.ValidateApplication("knowledge", "app.version", "auth.public_key", "postgres.dsn", "postgres.password", "etcd.username", "etcd.password", "nats.username", "nats.password", "nats.token", "object_storage.access_key", "object_storage.secret_key", "trace.headers", "trace.tls.key_file", "rpc.tls.key_file", "admin_http.tls.key_file", "identity_rpc.tls.key_file", "collaboration_rpc.tls.key_file"); err != nil {
		return Config{}, result, err
	}
	if document.Legacy() {
		...
		if err != nil {
			return Config{}, result, fmt.Errorf("encode knowledge dynamic configuration: %w", err)
		}
		decoder := yaml.NewDecoder(bytes.NewReader(contents))
		decoder.KnownFields(true)
		if err := decoder.Decode(&candidate); err != nil {
			return Config{}, result, fmt.Errorf("apply knowledge dynamic configuration: %w", err)
		}
	}
...
func cloneConfig(source Config) (Config, error) {
	...
	if err != nil {
		return Config{}, fmt.Errorf("clone knowledge configuration: %w", err)
	}
	clone := New()
	decoder := yaml.NewDecoder(bytes.NewReader(contents))
	decoder.KnownFields(true)
...
func applyEnvironmentOverrides(base Config) (Config, error) {
	...
	for _, key := range configurationKeys(reflect.ValueOf(base), "") {
		if err := v.BindEnv(key); err != nil {
			return Config{}, fmt.Errorf("bind knowledge environment variable for %q: %w", key, err)
		}
	}
	loaded := New()
	if err := v.UnmarshalExact(&loaded, viper.DecodeHook(configurationDecodeHook())); err != nil {
		return Config{}, fmt.Errorf("decode knowledge environment overrides: %w", err)
	}
	return loaded, nil
}
...
func readYAMLConfig(v *viper.Viper, path string) error {
	...
	case ".yaml", ".yml":
	default:
		return fmt.Errorf("read knowledge configuration %q: file extension must be .yaml or .yml", path)
	}
	contents, err := os.ReadFile(path)
	if err != nil {
		return fmt.Errorf("read knowledge configuration %q: %w", path, err)
	}
	v.SetConfigType("yaml")
	if err := v.ReadConfig(bytes.NewReader(contents)); err != nil {
		return fmt.Errorf("parse knowledge configuration %q: %w", path, err)
	}
	return nil
}
...
func configurationKeys(value reflect.Value, prefix string) []string {
	...
	var keys []string
	for index := 0; index < value.NumField(); index++ {
		name := mapstructureName(value.Type().Field(index))
		if name != "" {
