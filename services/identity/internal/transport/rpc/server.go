...
func NewRPCServer(
	...
	if ctx == nil || service == nil || telemetry == nil || metricsRegistry == nil || logger == nil {
		return nil, errors.New("create identity RPC server: context, service, tracing, metrics, and logger are required")
	}
...
func (s *RPCServer) Ready(ctx context.Context) error {
	if s == nil || s.server == nil || s.listener == nil {
		return errors.New("wait for identity RPC readiness: listener is not bound")
	}
	if ctx != nil {
