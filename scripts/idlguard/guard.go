...
func serviceFiles(root string) ([]string, error) {
	...
	for _, file := range files {
		ast, parseErr := parseIDL(root, filepath.Join(root, file))
		if parseErr != nil {
			return nil, parseErr
		}
...
func compareTrees(baseRoot, currentRoot string) error {
	...
	for _, relative := range baseFiles {
		...
		baseAST, parseErr := parseIDL(baseRoot, basePath)
		if parseErr != nil {
			return parseErr
		}
		currentAST, parseErr := parseIDL(currentRoot, currentPath)
		if parseErr != nil {
			return parseErr
		}
...
// compareGit materializes the base IDL tree from Git in an isolated temporary
// directory. It never checks out or mutates the caller's worktree.
func compareGit(repositoryRoot, baseRevision, currentRoot string) error {
	...
	verify := exec.Command("git", "-C", absRepository, "rev-parse", "--verify", baseRevision+"^{commit}")
	verifiedOutput, err := verify.Output()
	if err != nil {
		return fmt.Errorf("resolve IDL base revision %q: %w", baseRevision, err)
	}
	verifiedRevision := strings.TrimSpace(string(verifiedOutput))

	list := exec.Command("git", "-C", absRepository, "ls-tree", "-rz", "--name-only", verifiedRevision, "--", filepath.ToSlash(relativeRoot))
	output, err := list.Output()
	if err != nil {
		return fmt.Errorf("list IDLs at %s: %w", verifiedRevision, err)
	}
	...
	for _, rawPath := range bytes.Split(output, []byte{0}) {
		...
		show := exec.Command("git", "-C", absRepository, "show", verifiedRevision+":"+repositoryPath)
		content, showErr := show.Output()
		if showErr != nil {
			return fmt.Errorf("read %s at %s: %w", repositoryPath, verifiedRevision, showErr)
		}
...
func thriftFiles(root string) ([]string, error) {
	...
	for _, entry := range entries {
		...
		if entry.IsDir() {
			nested, nestedErr := thriftFiles(path)
			if nestedErr != nil {
				return nil, nestedErr
			}
			for _, file := range nested {
				files = append(files, filepath.Join(entry.Name(), file))
			}
			continue
		}
		if filepath.Ext(entry.Name()) == ".thrift" {
...
func parseIDL(root, path string) (*parser.Thrift, error) {
